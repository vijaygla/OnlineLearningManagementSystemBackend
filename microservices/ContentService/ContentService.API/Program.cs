...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "Content Service API", Version = "v1" });
...
using (var scope = app.Services.CreateScope())
{
    ...
    try 
    {
        Console.WriteLine("--- ☁️ Syncing Database (ContentService)... ---");
...
app.UseSwagger();
app.UseSwaggerUI(options => { options.SwaggerEndpoint("/swagger/v1/swagger.json", "Content Service API v1"); options.RoutePrefix = "swagger"; });
